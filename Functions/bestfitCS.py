...
def cutting_stock(cuts_dict):
    ...
    for stud, cuts in cuts_dict.items():
        ...
        # Dictionary to track the quantity of each cut used
        cut_usage = defaultdict(int)
        ...
        while flat_cuts:
            ...
            for cut in flat_cuts[:]:
                if cut <= remaining_length:
                    ...
                    cut_usage[cut] += 1  # Track the usage of this cut
        ...
        # Store the summary of cut usage
        cut_summary[stud] = dict(cut_usage)

    return results, waste_percentages, cut_summary
